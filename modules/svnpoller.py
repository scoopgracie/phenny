...
def loadRevisions(fn):
    ...
    with open(fn) as f:
        for line in f:
            ...
            if line:
                try:
                    repo, rev = line.split('\t', 2)
                except ValueError:
                    continue  # @@ hmm
                # []).append((teller, verb, timenow, msg))
                result.setdefault(repo, int(rev))
    return result
...
class SVNPoller:
    ...
    def check(self, revisions):
        ...
        print(str(self.latest_revision), str(
            self.last_revision), str(global_revisions))
