def sd(phenny, input):
    """.sd - short description of Apertium"""
...
def ld(phenny, input):
    """.ld - long description of Apertium"""
...
def when(phenny, input):
    ...
    phenny.say("RBMT gives the best results for closely related languages. Apertium is specifically superior to Google Translate in this scenario, because Google Translate uses English as an intermediate. For example, translating from Catalan to Spanish with Google Translate is actually translating from Catalan to English to Spanish. In Apertium, translation occurs directly between Catalan and Spanish.")
    phenny.say("Another advantage of RBMT is that it works for lesser-resourced languages. Neural machine tranlsation (NMT), used by Google Translate and many other systems, gives excellent results for languages with extremely large bilingual corpora, but breaks down for lesser-resourced languages. RBMT only requires someone who knows both languages and is willing to take the time to learn about Apertium and create the data.")
